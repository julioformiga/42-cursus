...
void	PhoneBook::set_information(void) {
	...
	else
	{
		std::cout << "Your phonebook is full.\nIf you want to add a new "\
					"contact, i am going to delete the first contact i "\
					"stored, index #1." << std::endl;
		std::cout << "If you want to proceed, press '1' and confirm with "\
					"'enter',\nif not press anything but '1' and confirm "\
					"with 'enter' to go back to main menu." << std::endl;
		...
		else if (input.compare("1") == 0)
		{
			...
			std::cout << "Back to main menu without deleting and\
...
void	PhoneBook::get_information() const {
	...
	if (this->_index == 0)
		std::cout << "\033[31mPlease add at least one contact before"\
						"searching.\033[0m" << std::endl;
	else
	{
		std::string input;
		std::cout << "Please tell me which contact index i should show you."\
					" (0 to quit searching)\nIndex: ";
		while (
			!(std::getline(std::cin, input)) || input.length() > 1
				...
				&& this->_full == false))
		{
			if (std::cin.eof() == true)
			{
				std::cout << "You Pressed ^D. "\
							"Exiting phonebook now." << std::endl;
				std::exit(0);
			}
			...
			{
				std::cin.clear();
				std::cout << "\033[31mOnly digits in range of "\
							"1 to 8 are allowed.\033[0m\n";
				std::cout << "Please tell me which contact i should "\
							"show you. (0 to quit searching)\nIndex: ";
			}
			else if (std::atoi(input.c_str()) - 1 >= this->_index
				&& this->_full == false)
			{
				std::cout << "\033[33mYou only have " << this->_index
					<< " Contacts saved.\033[0m" << std::endl;
				std::cin.clear();
				std::cout << "Please tell me which contact i should "\
							"show you. (0 to quit searching)\nIndex: ";
			}
